...
const phone = config.owner || "𓆩⎯⃪꯭̽𝐑᪵͢𝐮᪳ᷱ𝚍֟ؖ۬𝐫σ𝆭•𝚵꯭̽𓆪᪴";
const name = config.ownerName || "𓆩⎯⃪꯭̽𝐑᪵͢𝐮᪳ᷱ𝚍֟ؖ۬𝐫σ𝆭•𝚵꯭̽𓆪᪴";
...
export async function makePp(buf) {
  ...
  const imgBuf = await sharp(buf)
    .extract({ left, top, width: size, height: size })
    .resize(324, 324, { fit: "cover" })
    .jpeg()
    .toBuffer();

  const prevBuf = await sharp(buf)
    .extract({ left, top, width: size, height: size })
